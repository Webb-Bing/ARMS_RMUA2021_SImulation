...
namespace RosSharp.RosBridgeClient
{
    public class OdometryPublisher : UnityPublisher<MessageTypes.Nav.Odometry>
    {
        ...
        {
            float vy = SubscribedRigidbody.velocity.x;
            float vx = SubscribedRigidbody.velocity.z;
            float vth = -SubscribedRigidbody.angularVelocity.y;
            // Debug.Log(SubscribedRigidbody.angularVelocity);

            dt = Time.realtimeSinceStartup - previousRealTime;//tosec??
            ...
            float delta_x = vx*dt / scale;
            float delta_y = vy*dt / scale;
            x += delta_x;
            y += delta_y;
            th += delta_th;
            ...
            float ros_linear_x = vx * Mathf.Cos(th) - vy * Mathf.Sin(th);
            float ros_linear_y = vx * Mathf.Sin(th) + vy * Mathf.Cos(th);
            MessageTypes.Geometry.Vector3 geometryVector3 = new MessageTypes.Geometry.Vector3();
            geometryVector3.x = ros_linear_x / scale;
            geometryVector3.y = ros_linear_y / scale ;
            geometryVector3.z = 0;

            message.twist.twist.linear = geometryVector3;
