...
namespace RosSharp.RosBridgeClient
{
    public class LaserScanReader : MonoBehaviour
    {
        ...
        private void MeasureDistance()
        {
            ...
            raycastHits = new RaycastHit[samples];
            ranges = new float[samples];
            indexs = new int[samples];

            // Vector3 t_position = transform.position;
            // Vector3 t_forward = transform.forward;
            // Quaternion t_q = transform.rotation;


            // // not sure if the following parallel can improve fps or not
            // Parallel.ForEach(indexs, (i)=>
            //             {
            //                 rays[i] = new Ray(t_position, GetRayRotation(i) * t_forward);
            //                 directions[i] = Quaternion.Euler(-t_q.eulerAngles) * rays[i].direction;
            ...
            // {
            //     // the following line is not allowed to run in other thread than the master thread by unity
            ...
            //         // DebugLine.DrawRay(rays[i].origin, directions[i]*raycastHits[i].distance, Color.blue, 100);
            //     }
            //     else
            //     {
            //         // DebugLine.DrawRay(rays[i].origin, directions[i]*range_max, Color.blue, 100);
            //     }
            // }
